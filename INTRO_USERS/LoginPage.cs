...
namespace INTRO_USERS
{
    ...
    public partial class LoginPage : Form
    {
        ...
        public LoginPage()
        {
            ...
            textBoxPassword.MaxLength = 32;         //max textbox character count
        }
        ...
        private void buttonLogin_Click(object sender, EventArgs e)
        {
            ...
            //(2) SELECT statement getting all data from users, depending on whether it is a manager or programmer
            ...
            if (loggedIn)
            {
                ...
                initialiseTextBoxes();
                //If the login is sucessful it opens up the browse page 
                Hide();
                if (radioButtonManager.Checked == true)
                {
                    BrowsePage browsePage = new BrowsePage(username);
                    browsePage.ShowDialog();
                }
                else if (radioButtonEmployee.Checked == true)
                {
                    EmployeePage employeePage = new EmployeePage(username);
